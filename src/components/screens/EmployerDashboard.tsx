...
export const EmployerDashboard: React.FC<EmployerDashboardProps> = ({
	...
	const actualApplicants = useMemo(() => {
		if (!isLoggedIn || applications.length === 0) return [];

		return applications
			.map((application) => {
				if (application.job_id == null) return null;
				const candId = Number(application.candidate_id);
				const fromPool = candidates.find(
					(candidateItem) => Number(candidateItem.id) === candId,
				);
				const candidate =
					fromPool ||
					({
				...
				const appliedToJob = myJobs.find(
					(job) => Number(job.id) === Number(application.job_id),
				);
				if (!appliedToJob) return null;
				...
				return {
					...
					appliedAgo: application.applied_at
						? formatDistanceToNow(
								parseAppTimestamp(application.applied_at) ||
	...
	const directMessageLeads = useMemo(() => {
		if (!isLoggedIn) return [];
		const inApplications = new Set(
			actualApplicants.map((a: any) => Number(a.id)),
		);
		const seen = new Set<number>();
		const out: any[] = [];
		for (const conv of conversations || []) {
			if (conv.job_id != null && conv.job_id !== undefined) continue;
			const cid = Number(conv.candidate_id);
			if (!cid || seen.has(cid) || inApplications.has(cid)) continue;
			seen.add(cid);
			const fromPool = candidates.find((c) => Number(c.id) === cid);
			out.push(
				fromPool || {
					id: cid,
		...
		return out;
	}, [isLoggedIn, conversations, candidates, actualApplicants]);
	...
	const filteredApplicants = filterByJobId
		...
		: actualApplicants;
	const dashboardApplicants = filteredApplicants;
	const filterJobTitle = filterByJobId
		? jobs.find((j) => j.id === filterByJobId)?.title
		: null;

	// Unlocked talent tagged to the filtered job but NOT already in applicants (de-duplication)
	const filterJobApplicantIds = new Set(
		filteredApplicants.map((a: any) => Number(a.id)),
	);
	...
	return (
		<>
			<motion.div
				initial={{ opacity: 0 }}
				animate={{ opacity: 1 }}
				className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-12 sm:py-16 md:py-20 space-y-12 sm:space-y-16 md:space-y-24 mb-12 sm:mb-16 md:mb-20"
			>
					...
					<div className="p-6 sm:p-8 bg-gradient-to-r from-[#A63F8E]/5 to-[#148F8B]/5 rounded-[2rem] sm:rounded-[3rem] border border-[#A63F8E]/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
						<div className="space-y-2 flex-1">
							...
							<p className="text-sm sm:text-base text-gray-600 font-medium">
								No sign-up required to explore. Create an account and verify
								your business to post jobs and unlock profiles.
							</p>
						</div>
				...
				<div className="space-y-4 sm:space-y-5">
						...
						<div className="flex flex-col md:flex-row md:flex-wrap gap-3 md:gap-3 w-full">
							...
							{isLoggedIn && (
								...
								>
									<LogOut
										size={16}
										className="sm:w-4 sm:h-4 md:w-3.5 md:h-3.5"
									/>{" "}
									Log Out
								</Button>
							)}
					...
					{/* Business Profile Snapshot (includes verification status) */}
					{isLoggedIn && userProfile && (
						<div className="p-5 sm:p-4 rounded-[1.5rem] sm:rounded-[2rem] border border-gray-100 bg-white shadow-sm space-y-2">
							...
							<div
								className={`p-4 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center gap-3 ${isVerified ? "bg-[#A63F8E]/5 border-[#A63F8E]/20" : "bg-amber-50 border-amber-200"}`}
							>
								<div
									className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isVerified ? "bg-[#A63F8E]/10" : "bg-amber-100"}`}
								>
									{isVerified ? (
										<CheckCircle size={18} className="text-[#A63F8E]" />
									) : (
								...
								<div className="flex-1 space-y-1">
									<p
										className={`text-[11px] font-black uppercase tracking-widest ${isVerified ? "text-[#A63F8E]" : "text-amber-700"}`}
									>
							...
							<p className="text-sm text-gray-600 font-medium leading-relaxed">
								{userProfile.bio ||
									"Add your business description so candidates understand your team and culture."}
							</p>
						</div>
					)}
				</div>
				...
				{isLoggedIn && (
					<div className="-mt-8 sm:-mt-10 md:-mt-12 p-5 sm:p-6 md:p-7 bg-gray-900 text-white rounded-[2rem] sm:rounded-[2.5rem] space-y-4 shadow-2xl relative overflow-hidden">
						...
						<div className="activity-stats-grid gap-3 w-full">
				...
				<div className="-mt-10 sm:-mt-12 md:-mt-14 grid lg:grid-cols-2 gap-8 sm:gap-12 md:gap-16">
					<section className="space-y-8 sm:space-y-12">
						<div className="flex items-center justify-between">
							<h3 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight flex items-center gap-3 sm:gap-5">
								<Briefcase
									size={28}
									className="sm:w-9 sm:h-9 md:w-10 md:h-10 text-[#148F8B]"
								/>{" "}
								Your Jobs
							</h3>
							<Button
						...
						<div className="space-y-4 sm:space-y-6">
							{myJobs.map((j) => {
								const jobApplicantCount = actualApplicants.filter(
									(a) => a.appliedToJob?.id === j.id,
								).length;
								return (
									<div
										key={j.id}
										className="p-6 sm:p-8 md:p-10 bg-white border border-gray-100 rounded-[2.5rem] sm:rounded-[3rem] md:rounded-[3.5rem] shadow-sm space-y-6 sm:space-y-8"
									>
										<div className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-0">
											<div className="flex-1 min-w-0">
												<h4 className="text-xl sm:text-2xl md:text-3xl font-black tracking-tight break-words">
													{j.title}
												</h4>
												<p className="text-gray-400 font-black uppercase tracking-[0.3em] text-[10px] pt-1">
													{j.location} •{" "}
													{j.status === "filled" ? "FILLED" : "LIVE"} •{" "}
													{jobApplicantCount} applicant
													{jobApplicantCount !== 1 ? "s" : ""}
												</p>
												{Array.isArray(j.application_questions) &&
													j.application_questions.length > 0 && (
														<span className="inline-flex items-center gap-1 mt-1.5 px-2 py-1 bg-[#148F8B]/10 text-[#148F8B] rounded-lg text-[9px] font-black uppercase tracking-widest">
															...
															{j.application_questions.length} custom question
															{j.application_questions.length !== 1 ? "s" : ""}
														</span>
													)}
											</div>
											<Button
										...
										<div className="flex flex-col md:flex-row gap-3 md:gap-4">
											<Button
												variant="outline"
												className="w-full md:flex-1 h-12 sm:h-14 md:h-16 rounded-[1.2rem] border-gray-100 bg-white text-xs font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all duration-200"
												onClick={() => {
											...
											<Button
												variant="outline"
												className="w-full md:w-auto h-12 sm:h-14 md:h-16 px-5 rounded-[1.2rem] border-gray-100 bg-white text-xs font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all duration-200"
												onClick={() => onNavigate("candidates")}
											>
												Talent Pool
											</Button>
					...
					<section className="space-y-8 sm:space-y-12">
						<div className="space-y-4">
							<h3 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight flex items-center gap-3 sm:gap-5">
								<Star
									size={28}
									className="sm:w-9 sm:h-9 md:w-10 md:h-10 text-[#A63F8E]"
								/>{" "}
								Saved talent
							</h3>
							<p className="text-sm sm:text-base text-gray-600 font-medium">
								Organize saved candidates into job-specific sections so it’s
								clear who you’re considering for each role.
							</p>
							{(() => {
								const fromApplications = unlockedCandidates.filter((c: any) =>
									actualApplicants.some(
										(a: any) => Number(a.id) === Number(c.id),
									),
								).length;
								const fromPool = Math.max(
									0,
									unlockedCandidates.length - fromApplications,
								);
								return (
									<div className="flex flex-wrap gap-2">
										<span className="px-3 py-1.5 rounded-full bg-gray-100 text-gray-700 text-[10px] font-black uppercase tracking-widest">
											Total saved: {unlockedCandidates.length}
										</span>
						...
						{unlockedCandidates.length === 0 ? (
							<div className="p-12 sm:p-16 md:p-20 bg-[#F3EAF5]/30 rounded-[3rem] sm:rounded-[3.5rem] md:rounded-[4rem] border-4 border-dashed border-gray-100 text-center space-y-4 sm:space-y-6">
								<Users
									size={40}
									className="sm:w-11 sm:h-11 md:w-12 md:h-12 mx-auto text-gray-600"
								/>
								...
								<Button
									variant="outline"
									className="h-12 sm:h-14 px-6 sm:px-8 mx-auto hover:scale-105 active:scale-95 transition-all duration-200"
									onClick={() => onNavigate("candidates")}
								>
									Browse Candidates
								</Button>
							...
							<div className="grid gap-6">
								...
								{(() => {
									...
									if (untagged.length)
										sections.unshift({
											key: "untagged",
											title: "Untagged (needs organizing to specific job(s))",
											items: untagged,
										});
									...
									return sections.map((section) => (
										<div
											key={section.key}
											className={`space-y-4 rounded-[2rem] sm:rounded-[2.5rem] border-2 p-4 sm:p-6 ${
												section.jobId
													? "border-[#148F8B]/20 bg-[#148F8B]/5"
										...
										>
											<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
												<div className="space-y-1">
													...
													<div className="flex items-center gap-2">
														<span className="text-lg sm:text-xl font-black tracking-tight text-gray-900">
															{section.jobId
																? section.title
																: "Needs organizing into specific job(s)"}
														</span>
														{section.jobId && (
												...
												<span className="px-3 py-1.5 rounded-full bg-white/80 border border-gray-200 text-[10px] font-black uppercase tracking-widest text-gray-700">
													{section.items.length} candidate
														...
														>
															<div className="flex items-center gap-4 sm:gap-6">
																<div className="w-16 sm:w-20 h-16 sm:h-20 rounded-xl sm:rounded-2xl bg-gray-100 overflow-hidden relative shrink-0">
																	<ImageWithFallback
																		src={
																			cand.video_thumbnail_url || cand.thumbnail
																		}
				...
				{/* Message-only threads (no job application) — open profile to unlock */}
				{isLoggedIn && directMessageLeads.length > 0 && (
					<section className="space-y-4 sm:space-y-6">
						<div className="space-y-2">
							...
							<p className="text-sm text-gray-600 font-medium max-w-2xl">
								These people messaged you without applying to a specific job.
								Click their name or photo to open their profile and unlock to
								see full details.
							</p>
						</div>
						<div className="grid gap-3 sm:grid-cols-2">
							{directMessageLeads.map((c) => (
								<button
									key={c.id}
									type="button"
									onClick={() =>
								...
								>
									<div className="w-14 h-14 rounded-xl bg-gray-100 overflow-hidden shrink-0 ring-2 ring-transparent group-hover:ring-[#148F8B]/20">
										<ImageWithFallback
											src={c.video_thumbnail_url || c.thumbnail}
											alt=""
											className="w-full h-full object-cover"
										/>
									</div>
									<div className="min-w-0 flex-1 space-y-1">
										<p className="font-black text-gray-900 truncate group-hover:text-[#148F8B] transition-colors">
											{c.name || "Candidate"}
										</p>
										{c.display_title ? (
										...
										<p className="text-[10px] font-black uppercase tracking-widest text-[#A63F8E] flex items-center gap-1">
											<Lock size={10} aria-hidden />
											Open profile to unlock
										</p>
									</div>
								</button>
							))}
				...
				{isLoggedIn && (
					<section
						ref={applicantsRef}
						className="space-y-8 sm:space-y-12 scroll-mt-28"
					>
						<div className="space-y-4">
								...
								{filterByJobId ? (
									<div className="flex items-center gap-3 flex-wrap">
										...
										<span className="text-xs font-black uppercase tracking-widest text-[#148F8B]">
											{filterJobTalent.length} saved talent
										</span>
										...
										<span className="text-xs font-black uppercase tracking-widest text-gray-600">
											{filteredApplicants.length + filterJobTalent.length} total
										</span>
									</div>
								) : (
							...
							{filterJobTitle && (
								<div className="flex items-center gap-3 p-3 bg-[#148F8B]/5 rounded-xl border border-[#148F8B]/10">
									...
									<span className="text-xs font-bold text-[#148F8B] flex-1 truncate">
										Showing candidates for: {filterJobTitle}
									</span>
									<button
										type="button"
						...
						{filteredApplicants.length === 0 &&
							filterJobTalent.length === 0 &&
							(filterByJobId ? true : directMessageLeads.length === 0) && (
								<div className="p-12 sm:p-16 md:p-20 bg-white border-4 border-dashed border-gray-100 rounded-[3rem] sm:rounded-[3.5rem] md:rounded-[4rem] text-center space-y-4 sm:space-y-6">
									<Mail
										size={40}
										className="sm:w-11 sm:h-11 md:w-12 md:h-12 mx-auto text-gray-400"
									/>
									<p className="text-gray-400 font-black uppercase tracking-widest text-xs sm:text-sm">
										{filterByJobId
									...
									<p className="text-sm text-gray-500 font-medium">
										{filterByJobId
											? "Applicants and saved talent tagged to this job will appear here."
											: "Job applications show here. People who only messaged you appear under “From your messages” — tap them to open a profile and unlock."}
									</p>
								</div>
							)}

						{/* Applicants (single list — no locked gate) */}
						{dashboardApplicants.length > 0 && (
							<div className="space-y-4">
								<div className="flex items-center gap-3">
									...
									<h4 className="text-sm font-black uppercase tracking-widest text-[#A63F8E]">
										Applicants
									</h4>
								</div>
									...
									return (
										<motion.div
											key={
												...
												`${applicant.id}-${applicant.appliedToJob?.id ?? "g"}`
											}
										...
										>
												...
												<div className="flex-1 min-w-0 space-y-2">
													<div className="flex items-start justify-between gap-3">
														...
														<div className="flex items-center gap-1.5 shrink-0">
															{/* Status Badge */}
															<span
																className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-normal ${statusStyle.bg} ${statusStyle.text} whitespace-nowrap`}
															>
																{applicant.status}
															</span>
															<button
											...
											{(() => {
												...
												const jobQuestions: string[] =
													applicant.appliedToJob?.application_questions || [];
												const answers: any[] = Array.isArray(
													application?.question_answers,
												)
													? application.question_answers
													: jobQuestions.map((question: string) => ({
															question,
														}));
												...
												return (
													<div onClick={(e) => e.stopPropagation()}>
														<button
															...
															onClick={() =>
																setOpenAnswersId(isOpen ? null : applicant.id)
															}
																				...
																				>
																					<div className="w-14 h-9 rounded-lg bg-[#148F8B]/15 flex items-center justify-center shrink-0">
																						<Play
																							aria-hidden="true"
																							size={14}
																							className="text-[#148F8B] fill-[#148F8B]"
																						/>
																					</div>
																				...
																				>
																					<div className="w-14 h-9 rounded-lg bg-[#780262]/15 flex items-center justify-center shrink-0">
																						<Play
																							aria-hidden="true"
																							size={14}
																							className="text-[#780262] fill-[#780262]"
																						/>
																					</div>
											...
											<div className="px-5 sm:px-6 pt-2 pb-10 flex flex-wrap gap-3 justify-center">
												...
												{applicant.phone ? (
													...
													<button
														type="button"
														disabled
														className="px-8 h-12 rounded-2xl bg-gray-100 text-gray-300 flex items-center justify-center gap-2 font-black text-sm uppercase tracking-wide cursor-not-allowed"
													>
														<Phone aria-hidden="true" size={16} />
														Call
													</button>
						...
						{/* Saved talent tagged to this job (unified view only) */}
						{filterByJobId && filterJobTalent.length > 0 && (
							<div className="space-y-4">
								<div className="flex items-center gap-3">
									...
									<h4 className="text-sm font-black uppercase tracking-widest text-[#148F8B]">
										Saved talent — tagged to this job
									</h4>
								</div>
								{filterJobTalent.map((cand: any, i: number) => {
									const taggedJobIds = candidateJobLinks[String(cand.id)] || [];
									const jobsTagged = jobs.filter((j: any) =>
										taggedJobIds.some(
									...
									return (
										...
										>
													...
													<div className="flex items-center gap-2">
														...
														<div className="flex flex-wrap gap-1">
															{jobsTagged.slice(0, 3).map((j: any) => (
																<span
																	key={j.id}
																	className="text-xs font-black text-[#148F8B] truncate"
																>
																	{j.title}
																</span>
															))}
											...
											{(contactMethod || employerNotes || contactNotes) && (
												<div
													className="px-5 sm:px-6 pb-4 space-y-3 border-t border-gray-50 pt-4"
													onClick={(e) => e.stopPropagation()}
												>
													<h5 className="text-xs font-black text-gray-400 uppercase tracking-widest">
														Review Notes
						...
						>
							...
							>
								<source
									src={videoLightbox}
									type={
										videoLightbox.includes(".webm") ? "video/webm" : "video/mp4"
									}
								/>
							</video>
						</div>

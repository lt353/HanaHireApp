...
import { MessageSquare, Send, Lock, ChevronLeft, Building2, User, Mail, LayoutDashboard, Check, CheckCheck, Mic, Pencil, Trash2 } from "lucide-react";
...
function OtherPartyAvatar({
  ...
  if (initials?.trim()) {
    return (
      <div className={`${className} bg-[#148F8B]/10 text-[#148F8B] font-black text-sm`}>
  ...
  function renderInline(text: string): React.ReactNode {
    ...
    while ((m = re.exec(text)) !== null) {
      if (m.index > lastIndex) {
        parts.push(<span key={`${keyPrefix}-${keyIdx++}`}>{text.slice(lastIndex, m.index)}</span>);
      }
      if (m[1] !== undefined) {
        parts.push(<strong key={`${keyPrefix}-${keyIdx++}`}>{renderInline(m[1])}</strong>);
      } else if (m[2] !== undefined) {
        parts.push(<s key={`${keyPrefix}-${keyIdx++}`}>{renderInline(m[2])}</s>);
      } else if (m[3] !== undefined) {
        parts.push(<em key={`${keyPrefix}-${keyIdx++}`}>{renderInline(m[3])}</em>);
      } else if (m[4] !== undefined && m[5] !== undefined) {
        parts.push(
          <a key={`${keyPrefix}-${keyIdx++}`} href={m[5]} target="_blank" rel="noopener noreferrer" className="text-[#148F8B] underline font-medium">{m[4]}</a>
        );
      }
      lastIndex = re.lastIndex;
    }
    if (lastIndex < text.length) {
      parts.push(<span key={`${keyPrefix}-${keyIdx++}`}>{text.slice(lastIndex)}</span>);
    }
  ...
  while (i < lines.length) {
    ...
    if (bulletMatch) {
      ...
      while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) {
        listItems.push(<li key={`${keyPrefix}-li-${keyIdx++}`}>{renderInline(lines[i].replace(/^\s*[-*]\s+/, ""))}</li>);
        i++;
      }
      out.push(<ul key={`${keyPrefix}-ul-${keyIdx++}`} className="list-disc pl-5 my-1 space-y-0.5">{listItems}</ul>);
      if (i < lines.length) out.push(<br key={`${keyPrefix}-br-${keyIdx++}`} />);
      continue;
    }
    if (numMatch) {
      const listItems: React.ReactNode[] = [];
      while (i < lines.length && /^\s*\d+\.\s+/.test(lines[i])) {
        listItems.push(<li key={`${keyPrefix}-oli-${keyIdx++}`}>{renderInline(lines[i].replace(/^\s*\d+\.\s+/, ""))}</li>);
        i++;
      }
      out.push(<ol key={`${keyPrefix}-ol-${keyIdx++}`} className="list-decimal pl-5 my-1 space-y-0.5">{listItems}</ol>);
      if (i < lines.length) out.push(<br key={`${keyPrefix}-br-${keyIdx++}`} />);
      continue;
    }
    out.push(<span key={`${keyPrefix}-${keyIdx++}`}>{renderInline(line)}</span>);
    if (i < lines.length - 1) out.push(<br key={`${keyPrefix}-br-${keyIdx++}`} />);
    i++;
  }
...
function markdownToHtml(md: string): string {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*[-*]\s+/.test(line)) {
      out.push("<ul>");
      while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) {
        out.push("<li>" + inlineToHtml(lines[i].replace(/^\s*[-*]\s+/, "")) + "</li>");
        i++;
      }
      out.push("</ul>");
    ...
    if (/^\s*\d+\.\s+/.test(line)) {
      out.push("<ol>");
      while (i < lines.length && /^\s*\d+\.\s+/.test(lines[i])) {
        out.push("<li>" + inlineToHtml(lines[i].replace(/^\s*\d+\.\s+/, "")) + "</li>");
        i++;
      }
      out.push("</ol>");
      if (i < lines.length) out.push("<br>");
  ...
  function walk(node: Node): string {
    ...
    if (tag === "ul") {
      return Array.from(el.children)
        .map((child) => (child.tagName.toLowerCase() === "li" ? "- " + walk(child) : walk(child)))
        .join("\n");
    }
    if (tag === "ol") {
      return Array.from(el.children)
        .map((child, idx) =>
          child.tagName.toLowerCase() === "li" ? `${idx + 1}. ${walk(child)}` : walk(child)
        )
        .join("\n");
    }
  ...
  const startListening = () => {
    ...
    try {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        if (finalTranscript) {
          const next = (draft ? `${draft} ${finalTranscript}` : finalTranscript).trim();
          setDraft(next);
        } else if (interimTranscript) {
          const next = (draft ? `${draft} ${interimTranscript}` : interimTranscript).trim();
          setDraft(next);
        }
      };
                ...
                {canReadSelected && (
                  <>
                    <div
                      ...
                      className={cn("flex-1 overflow-y-auto overflow-x-hidden px-5 py-4 space-y-4 min-h-0 flex flex-col", styles.bubbleList)}
                    >
                      {isLoadingMessages ? (
                        ...
                        messages.map((msg, i) => {
                          ...
                          const noTail = i < messages.length - 1 && messages[i + 1].from_email.toLowerCase() === msg.from_email.toLowerCase();
                          return (
                            <div
                              key={msg.id}
                              style={{
                            ...
                            >
                              <div
                                ...
                                style={{
                                  maxWidth: editingMessageId === msg.id ? "100%" : "78%",
                                  width: editingMessageId === msg.id ? "100%" : undefined,
                                  minWidth: 0,
                                }}
                              >
                                  {editingMessageId === msg.id && isMe && onEditMessage ? (
                                    <div className="space-y-2 w-full min-w-0">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <button
                                          type="button"
                                          onClick={async () => {
                                            if (!editingBody.trim() || savingEdit) return;
                                            setSavingEdit(true);
                                            try {
                                              await onEditMessage(msg.id, editingBody);
                                              setEditingMessageId(null);
                                              setEditingBody("");
                                            } finally {
                                              setSavingEdit(false);
                                    ...
                                    <>
                                      <p className="text-sm font-medium" style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{renderMessageBody(msg.body, `b-${msg.id}`)}</p>
                                      <div className="flex flex-wrap items-center gap-x-4 gap-y-0 mt-2 text-gray-800">
                                        <span className="text-[10px]" title={parseUtcTimestamp(msg.sent_at)?.toISOString() ?? msg.sent_at}>
                                          {format(parseUtcTimestamp(msg.sent_at) ?? new Date(0), "MMM d, h:mm a")}
                                        </span>
                                        {isMe && (
                                          <span className="flex items-center gap-0.5 text-[10px] ml-2">
                                            {msg.is_read ? (
                                              <>
                                                ...
                                                <span>Read </span>
                                                {msg.read_at && (
                                                  <span title={parseUtcTimestamp(msg.read_at)?.toISOString() ?? msg.read_at}>
                                                    {" \u00A0"}&middot; {format(parseUtcTimestamp(msg.read_at) ?? new Date(0), "h:mm a")}
                                                  </span>
                                                )}
                                              </>
                                            ) : (
                                        ...
                                        {isMe && onDeleteMessage && (() => {
                                          const sentAt = parseUtcTimestamp(msg.sent_at);
                                          const hoursAgo = sentAt ? (Date.now() - sentAt.getTime()) / (60 * 60 * 1000) : 0;
                      ...
                      <div className="flex gap-3 items-end">
                        <div className="flex-1 flex flex-col min-w-0 messages-composer-quill">
                          <ReactQuill
                            ...
                            onChange={(content) => setDraft(htmlToMarkdown(content || ""))}
                          ...
                          >
                            {isListening ? (
                              <span
                                ...
                                style={{ width: 14, height: 14, backgroundColor: "#ef4444" }}
                                aria-hidden
                              />
                            ) : (
